...
#### Originally written by Paul Schuetze (taken from gitlab.cern.ch/pschutze/PixelROCplotter/-/blob/master/rocs.py)####
#### Updated by Guru Reddy and Athar Ahmad #####
#####################################################
### GLOBAL VARS
#
rocsInCol = 2
      ...
      for hists in [self.barrelHists, self.forwardHists]:
          for hist in hists:
              if hist.GetEntries():
                  ...
                  colorString = "_coded" if colorCoded else "_pixelalive" if pixelAlive else ""
                  suffix = "_{}".format(inputFileName[:-4]) if useFileSuffix else ""
                  output_path = os.path.join(output_dir, "{}{}{}.png".format(hist.GetName(), colorString, suffix))

                  print("Saving histogram to:", output_path)
  ...
  for item in inputFile:
    ...
    if len(inputs) >= 4:
      if item.startswith("Bad"):
        detElements = inputs[2].split("_")
        roc = int(detElements[6][3:])

        if len(inputs) == 3:
          reason = str(inputs[4]).lower().strip()
        else:
          reason="unknown"
